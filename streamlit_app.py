...
if not st.session_state.auth:
    ...
    if page == "📊 Advisor":
        ...
        if markets:
            for m in markets:
                st.markdown(f'<div class="purple-box" style="border-color:{m["color"]};"><h2>{m["name"]}</h2><h1 style="color:{m["color"]};">{m["signal"]}</h1><h3>₹{m["price"]:,.0f}</h3></div>', unsafe_allow_html=True)

    elif page == "🏠 Dashboard":
        st.title("Dashboard")
        df = pd.read_csv(f"{CSV_URL}&r={random.randint(1,999)}")
        st.write("Summary details here...")

    elif page == "💰 Add Entry":
        st.title("Smart Voice Entry 🎙️")
        v_raw = speech_to_text(language='ml', key='voice_orig')
        ...
        st.write("Track your debts here.")
