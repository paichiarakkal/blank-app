...
st.set_page_config(page_title="PAICHI GOLD v6.0", layout="wide")
...
if not st.session_state.auth:
    ...
    else: menu_options = ["🏠 Dashboard", "💰 Add Entry", "📊 Advisor", "📊 Report", "🔍 History", "🤝 Debt Tracker"]
    ...
    if page == "🏠 Dashboard":
        ...
        st.markdown(f"""
            <div style="background: rgba(255, 255, 255, 0.05); padding: 30px; border-radius: 20px; border-left: 10px solid #FFD700; text-align: center; margin-bottom: 30px;">
                <p style="color: #E0B0FF !important; font-size: 20px; margin-bottom: 10px;">Net Balance</p>
    ...
    # --- 💰 ADD ENTRY ---
        ...
        if st.session_state.last_wa_url:
            ...
            st.markdown(f'<a href="{st.session_state.last_wa_url}" target="_blank"><button style="width:100%; background:#25D366; color:white; border:none; padding:15px; border-radius:10px; font-weight:bold; cursor:pointer; margin-bottom:20px;">🚀 SEND WHATSAPP NOTIFICATION</button></a>', unsafe_allow_html=True)
            if st.button("➕ Add New Entry"):
                st.session_state.last_wa_url = None
                st.rerun()
        else:
            v_raw = speech_to_text(language='ml', key='voice_v6_0')
            v_cat, v_amt, v_desc = process_voice(v_raw)
            
            with st.form("entry_form", clear_on_submit=True):
                it = st.text_input("Description", value=v_desc)
                # Amount ബോക്സിൽ 0.0 ഒഴിവാക്കി
                ...
                if st.form_submit_button("SAVE DATA"):
                    try:
                        # നമ്പറിലെ സ്പേസ് കളഞ്ഞ് ക്ലീൻ ചെയ്യുന്നു
                        clean_am = am_str.strip().replace(',', '')
                        am = float(clean_am)
                        ...
                        if it and am > 0:
                            ...
                            st.rerun()
                        else: st.error("Description-ഉം Amount-ഉം നൽകുക!")
                    except: 
                        st.error("Amount ബോക്സിൽ നമ്പർ മാത്രം നൽകുക!")
    ...
    # മറ്റ് ഭാഗങ്ങൾ (Advisor, Report) ഇവിടെ പഴയപോലെ ചേർക്കാം.
