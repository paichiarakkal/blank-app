...
st.set_page_config(page_title="PAICHI GOLD v5.6", layout="wide")
...
st.markdown("""
    ...
    /* ആ വെളുത്ത ചെറിയ ബോക്സ് പൂർണ്ണമായും ഒഴിവാക്കാൻ */
    .element-container:has(iframe[title="streamlit_mic_recorder.speech_to_text"]) + div { display: none !important; }
    </style>
...
if not st.session_state.auth:
    ...
    if page == "💰 Add Entry":
        ...
        v_raw = speech_to_text(language='ml', key='voice_v5_6')
        v_cat, v_amt, v_desc = process_voice(v_raw)
        
        with st.form("entry_form", clear_on_submit=True):
            it = st.text_input("Description", value=v_desc)
            # 🟢 ഇവിടെ മാറ്റം വരുത്തി: 0.0 ഒഴിവാക്കാൻ text_input ഉപയോഗിച്ചു
            ...
            if st.form_submit_button("SAVE & NOTIFY"):
                try:
                    ...
                    else: st.error("Please enter item and amount!")
                except ValueError: st.error("Amount ബോക്സിൽ നമ്പർ മാത്രം നൽകുക!")
    ...
    # ബാക്കി Advisor, Report എല്ലാം പഴയപോലെ...
