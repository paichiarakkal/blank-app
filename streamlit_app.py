...
st.set_page_config(page_title="PAICHI v16.0", layout="wide")
...
def get_signals():
    ...
    for name, sym in {"Nifty 50": "^NSEI", "Bank Nifty": "^NSEBANK", "Crude Fut": "CL=F"}.items():
        try:
            ...
            if name == "Crude Fut": last_p *= 96 # Approx price in INR
...
if not st.session_state.auth:
    ...
    if page == "📊 Advisor":
        ...
        with st.form("f"):
            ...
            cat = st.selectbox("Cat", ["Trading", "Food", "Rent", "Salary", "Other"])
            ty = st.radio("Type", ["Debit", "Credit"], horizontal=True)
            if st.form_submit_button("SAVE"):
                requests.post(FORM_API, data={"entry.1044099436": datetime.now().strftime("%d/%m/%Y"), "entry.2013476337": f"[{cat}] {it}", "entry.1460982454": am if ty=="Debit" else 0, "entry.1221658767": am if ty=="Credit" else 0})
